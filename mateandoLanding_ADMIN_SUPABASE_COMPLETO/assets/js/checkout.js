...
function initSupabaseClient() {
  if (!STORE_CONFIG.SUPABASE_ENABLED) return;
  if (!window.supabase) {
    console.warn("Supabase SDK no está cargado.");
    return;
  }
  if (!STORE_CONFIG.SUPABASE_URL || !STORE_CONFIG.SUPABASE_ANON_KEY || String(STORE_CONFIG.SUPABASE_URL).includes("PEGAR_")) {
    console.warn("Supabase no configurado. El pedido no se guardará en la base de datos.");
    return;
  }
  supabaseClient = window.supabase.createClient(STORE_CONFIG.SUPABASE_URL, STORE_CONFIG.SUPABASE_ANON_KEY);
}

async function saveOrderToSupabase(order, totals, message) {
  if (!supabaseClient) return { ok: false, message: "Supabase desactivado o sin configurar." };
  ...
  if (error) {
    ...
    return { ok: false, message: error.message || "No se pudo guardar el pedido en Supabase." };
  }
  return { ok: true, message: "Pedido guardado en Supabase." };
}
...
async function handleOrder(event) {
  ...
  await saveOrderToSupabase(order, totals, message);
